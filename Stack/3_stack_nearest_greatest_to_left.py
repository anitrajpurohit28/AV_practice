# Nearest Greatest to Left OR Next largest Element to Left
...
#
# 4. We do NOT need to reverse the output since we traversed from left to right;
...
def nearest_greatest_to_left(arr: list):
    ...
    stack = []          # Will be used to keep track of elements on left\

    # Traverse the aray from left to right to left as asked in the problem
    for item in arr[::]:
        logger.info("current item: %s\nOutput: %s\nStack: %s", item, output_list, stack)
        # if stack is empty, append -1; This will be hit for first element
        if len(stack) == 0:
            logger.debug("First iteration; stack is empty. append -1 to output and %s to stack", item)
        ...
        # if top of stack element is greater than current item, append s[top] to output
            ...
            if len(stack) == 0:
                ...
                logger.debug("We found a bigger element on top of stack; append %s to output", stack[-1])
                output_list.append(stack[-1])
...
a1 = [1, 3, 2, 4]
res = nearest_greatest_to_left(a1)
print(res)
print("--------")

a2 = [1, 3, 0, 0, 1, 2, 4]
res = nearest_greatest_to_left(a2)
print(res)
print("--------")
